...
int tv_uring_read(struct io_uring *ring, int fd, void *buf, size_t len, off_t offset) {
    ...
    /* user_data=-1: read path does not use CQE user_data for scheduling;
     * set to sentinel so any accidental deref is deterministic, not aliased
     * to a real TV_STRIPE_BUF pointer. */
